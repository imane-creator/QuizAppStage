...
mock.onPost('/jwt/register').reply(request => {
  if (request.data.length > 0) {
    const { email, password, fullName } = JSON.parse(request.data)
    const isEmailAlreadyInUse = users.find(user => user.email === email)
    const isfullNameAlreadyInUse = users.find(user => user.fullName === fullName)
    const error = {
      email: isEmailAlreadyInUse ? 'This email is already in use.' : null,
      fullName: isfullNameAlreadyInUse ? 'This fullName is already in use.' : null
    }

    if (!error.fullName && !error.email) {
      const { length } = users
      let lastIndex = 0
      if (length) {
